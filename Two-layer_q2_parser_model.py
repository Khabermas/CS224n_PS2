...
class ParserModel(Model):
	...
	V = initializer((self.config.hidden_size, self.config.hidden_size)
	...
	h2 = tf.nn.relu(tf.matmul(h_drop, V) + b2)
	h2_drop = tf.nn.dropout(h2, self.dropout_placeholder)
	pred = tf.matmul(h2_drop, U) + b3
	# l2 regularization 
	l2reg = self.config.l2reg
	self.l2_loss = l2reg * (tf.nn.l2_loss(W) + tf.nn.l2_loss(U) + tf.nn.l2_loss(V))
        ### END YOUR CODE
        return pred
